...
if($received_data->action == 'new_registration') {
  ...
  if ($resultTel != $tel) {
      if($user_password_hashed == $user_password_confirmed_hashed){
          ...
          $statement->execute();
          echo json_encode("OK");
      } else { echo json_encode("Mauvais mot de passe"); }
      while($row = $statement->fetch(PDO::FETCH_ASSOC)) {
          $data[] = $row;
      }
  } else { echo json_encode("Vous avez déjà un compte"); }
}
...
if($received_data->action == 'verif_login') {
  ...
  if($resultTel == $user_tel){
      ...
      if ($user_password_hashed === $resultPassword) {
          echo json_encode("OK");
      }  else { echo json_encode("Mauvais password");}
  } else { echo json_encode("Mauvais tel ou password"); }
}
...
if($received_data->action == 'fetch_edit_password') {
  ...
  $query3 = "SELECT tel FROM `users` WHERE tel= '$user_tel'";
  $statement3 = $connect->prepare($query3);
  $statement3->execute();
  $row2 = $statement3->fetch(PDO::FETCH_ASSOC);
  $resultTel = $row2['tel'];

  if($resultTel == $user_tel) {
    if($resultPassword == $old_password_hashed){
      if($new_password_hashed == $new_password_confirmed_hashed){
          $query = "UPDATE `users` SET `password` = '$new_password_hashed', `date_modification` = NOW() WHERE tel= '$user_tel';";
          $statement = $connect->prepare($query);
          $statement->execute();
